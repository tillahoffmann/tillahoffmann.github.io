...
def generate_data(
    n=1000, p=2, seed=1, theta=None, theta_mean=1, theta_std=1, X_mean=3, X_std=1
):
    """
    Generate synthetic data for linear regression. Default parameters are chosen for
...
def plot_data(X, y, theta, ax=None):
    ...
    ax.scatter(X[:, 1], y, marker=".", alpha=0.5)
...
def evaluate_elbo_landscape(X, y, mu1, mu2):
    ...
    """
    return np.reshape(
        [evaluate_elbo(X, y, (_mu1, _mu2)) for _mu1 in mu1 for _mu2 in mu2],
        (len(mu1), len(mu2)),
    ).T
